...
namespace classroom_scraper
{
    class OAuthSignIn
    {
        ...
        */
        ...
        public OAuthSignIn(string IDClient, string secretClient)
        {
            clientID = IDClient;
        ...
        public async Task<string> doOAuth(string scopes = "")
        {
            ...
            string authorizationRequest = string.Format("{0}?response_type=code&scope=openid%20profile%20{6}&redirect_uri={1}&client_id={2}&state={3}&code_challenge={4}&code_challenge_method={5}",
            ...
            {
                // broken link opening in windows...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    authorizationRequest = authorizationRequest.Replace("&", "^&");
                    System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo("cmd", $"/c start {authorizationRequest}") { CreateNoWindow = true });
                }
            ...
            string responseString = string.Format("<!DOCTYPE html><html><head><script>setTimeout(\"window.close()\", 3000)</script></head><body>Please return to the app.</body></html>");
